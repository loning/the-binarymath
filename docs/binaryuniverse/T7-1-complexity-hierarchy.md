...
```
$d(S) = \min\{n : S\text{可以通过}n\text{次自指操作从基础串生成}\}$
```
...
**定义**：复杂度类$C_n$定义为：
```
$C_n = \{S : d(S) = n \text{且} L_\phi(S) \leq p(n)\}$
```
其中$p(n)$是关于$n$的多项式，$L_\phi(S)$是$S$的φ-长度。
...
对于任意n ≥ 0，存在严格包含关系：
```
$C_0 \subset C_1 \subset C_2 \subset \ldots \subset C_n \subset C_{n+1} \subset \ldots$
```

且对于每个$n$，存在问题$P_n \in C_{n+1} - C_n$。
...
2. **归纳步骤**：
   假设$C_n \subset C_{n+1}$，需证明存在$P_{n+1} \in C_{n+2} - C_{n+1}$
   
   构造问题$P_{n+1}$："判断串$S$是否属于$C_{n+1}$"
   - 这需要模拟所有n+1阶自指操作
   - 根据对角化论证，这至少需要n+2阶自指
        - 因此$P_{n+1} \in C_{n+2} - C_{n+1}$
...
### 5.1 复杂度跳跃
**推论**：不存在复杂度类之间的连续过渡。系统要么在$C_n$中，要么至少在$C_{n+1}$中。

### 5.2 最优算法的存在性
**推论**：对于每个复杂度类$C_n$，存在最优算法，其φ-长度达到理论下界。
...
### 7.3 宇宙的计算结构
整个宇宙可以看作这个复杂度层级的物理实现，从基本粒子（$C_0$）到生命（$C_n$）再到意识（$C_m$，$m>n$）。
...
**证明**：
采用对角化方法。设$M_n = \{M_1, M_2, \ldots\}$是所有n阶复杂度的机器枚举。

构造机器D：
```
D(i) = {
  如果$M_i(i)$停机且输出0，则输出1
  如果$M_i(i)$停机且输出1，则输出0
  如果$M_i(i)$不停机，则输出0
}
```

D需要模拟所有n阶机器，这至少需要n+1阶复杂度。
但D不能在$M_n$中，否则存在i使得$D = M_i$，导致矛盾。
因此$D \in C_{n+1} - C_n$。□

### 8.2 φ-长度增长

**引理**：若$S \in C_{n+1} - C_n$，则$L_\phi(S) \geq \phi^n$
...
### 9.2 层级跳跃检测
设计实验检测算法从$C_n$跳跃到$C_{n+1}$的精确时刻。
