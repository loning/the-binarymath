...
**定义3.1bis** (φ-边界算子): 对于n-链c = ∑_{i} a_i e^n_i ∈ C^φ_n(M)，

$$
∂^φ_n(c) = φ^{-n/2} \sum_{i=1}^{F_{n+2}} (-1)^i \frac{F_{i+n+1}}{F_{n+4}} \cdot ∂_{std}(e^n_i)
$$

其中∂_{std}是标准边界算子，{e^n_i}是C^φ_n的Fibonacci权重基。
