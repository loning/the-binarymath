...
在collapse-aware框架中，自由节点的定义是：

> **自由节点 ≡ 在约束空间中，观察者注意力张量可以分配在多个φ-trace路径上的信息节点**
...
如果约束空间允许观察者在某些节点上分配注意力到多个路径，则存在无穷多个这样的自由节点，且𝔽在ℕ⁺中稠密。
...
在约束空间中，自由节点允许观察者将注意力分配到多个可能的路径。但在实际测量时，必须选择一个具体的路径。这个选择过程就是collapse。
...
**定义 4.4（共享自由节点）**
当两个或多个量子系统在约束空间中共享同一个自由节点的路径选择时，它们变得纠缠。
...
**预言1：量子不确定性与路径熵**
量子系统的不确定性应该正比于其在约束空间中对应的自由节点的路径熵。
...
2. **发现了自由节点的级联效应**：约束空间的结构产生无限的选择可能
