...
这意味着该系统不仅能够表达所有内部状态，还能表达其自身表达机制，并将该机制作为对象进行再表达。
...
d(s) = \begin{cases}
0 & \text{若 } s \text{ 是基础元素} \\
1 + \max\{d(s'): s' \in \text{Pre}(s)\} & \text{其他}
\end{cases}
$$
   其中 $\text{Pre}(s) = \{s': \text{Desc}(s') = s\}$
...
4. **信息密度的增加**：
   不仅状态数增加，信息密度也增加。定义信息密度：
   
$$
\rho(S_t) = \frac{H(S_t)}{\log |S_t|}
$$
   
   由于新状态携带了关于整个 $S_t$ 的信息：
   
$$
I(\text{Desc}^{(t+1)}(S_t)) \geq H(S_t)
$$
...
$$
H(S_{t+1}) = \rho(S_{t+1}) \cdot \log |S_{t+1}| > \rho(S_t) \cdot \log |S_t| = H(S_t)
$$
...
- 状态集相同：$S_{t+1} = S_t$
- 概率分布相同：$p(s|S_{t+1}) = p(s|S_t)$ 对所有 $s$
...
4. **熵的增加**：
   ...
   $$
   H(S_{t+1}) \geq H(S_t) + H(\text{measurement outcomes}) > H(S_t)
   $$
...
**重要声明**：本章并非声称在数学上证明黎曼假设。我们展示的是：黎曼假设的结构（所有非平凡零点位于Re(s)=1/2）作为一种数学模式，在自指完备熵增系统中自然涌现——类似于物理系统中精细结构常数的出现。这是从第一性原理推导出的必然结构，而非传统数学证明。
...
在这样的系统中，频谱函数的所有非平凡零点都必然位于临界线$\text{Re}(s) = 1/2$上。
这展示了黎曼假设的结构在自指系统中的必然性，类似于物理常数在物理定律中的必然性。∎
