...
$$
  1. **完整性**：$\forall s \in S: \text{Desc}(s) \text{ 能区分 } s \text{ 与任何其他状态}$
     即：$\forall s_1, s_2 \in S: s_1 \neq s_2 \Rightarrow \text{Desc}(s_1) \neq \text{Desc}(s_2)$
...
3. **时间存在**：$\exists \tau: \mathcal{S} \times \mathcal{S} \to \mathbb{R}^+$
4. **信息涌现**：$\exists I: \mathcal{S} \to \mathcal{I}$
5. **观察者存在**：$\exists O \subseteq S: O \times S \to \mathcal{M}$

**证明略缩**：由于篇幅限制，此处给出简化证明：

(1)⇒(2)：熵增意味着新描述的增加，故状态不能相同。
(2)⇒(3)：不对称变化定义了时间方向。
(3)⇒(4)：时间流逝产生了可区分的记录序列。
(4)⇒(5)：信息必须被"识别"才有意义，识别机制即观察者。
(5)⇒(1)：观察者的任何观察行为都会产生新的记录，增加系统熵。
...
**命题1.3（符号系统等价性假设）**
我们假设φ-表示系统与传统数学在表达能力上等价，基于以下观察：
1. 两者都是离散符号系统
2. 两者都通过有限操作定义数学对象
3. 两者都无法表示"不可区分的信息"
...
$$
这是定义性等价，不是额外公理。声称存在"不可表示的信息"就像声称存在"未婚的已婚者"——逻辑上自相矛盾。
...
3. **权重的涌现机制**：
   ...
   **证明**：考虑选择第$k$层描述的"成本"：
   - 计算成本：递归$k$次需要时间$\tau_k \propto k$
   - 存储成本：保存$k$层信息需要空间$S_k \propto k$
   
   **关键推导**：系统必须在熵增约束下完成观察。设系统可用的"资源"有限，总成本函数为：
   
$$
C_k = \beta_1 \tau_k + \beta_2 S_k = (\beta_1 + \beta_2) k = \alpha k
$$
   
   其中$\alpha = \beta_1 + \beta_2$是系统参数。
...
- 根据定理3.1，观察者涌现导致特定的错误模式
- 理论推论：φ-表示可能更自然地处理递归性错误
...
4. **稳定性需求 → 频率平衡 → 类黎曼结构**
   - 熵增与结构保持的矛盾
   - 频率平衡作为解决方案
   - 临界线$\text{Re}(s)=1/2$的出现（与黎曼假设的形式相似性）
