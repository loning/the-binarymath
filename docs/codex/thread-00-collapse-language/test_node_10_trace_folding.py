...
class TestTraceFolding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Valid trace: 00 01 00 → "000100" (no "11")
        self.trace1 = Trace([
            CollapseSymbol("00"),
            CollapseSymbol("01"),
            CollapseSymbol("00")
    ...
    def test_insertion_fold(self):
        ...
        self.assertEqual(str(folded), "10 00 01 00")
        
        # Insert in middle would create "11", so should fail
        with self.assertRaises(ValueError):
            FoldingOperations.insertion_fold(self.trace1, insert, 2)
        
        # Insert at end
        folded = FoldingOperations.insertion_fold(self.trace1, insert, 3)
        self.assertEqual(str(folded), "00 01 00 10")
    ...
    def test_unfold_operation(self):
        ...
        self.assertEqual(str(patterns[1]), "01 00")
    
    def test_information_density(self):
        """Test that folding increases information density"""
        original_length = len(self.trace1)
        
        # Test pattern preservation through folding
        folded = FoldingOperations.recursive_fold(self.trace1, 1)
        
        # The folded trace contains the original as a subpattern
        patterns = FoldingOperations.unfold(folded, original_length)
        
        # At least one pattern should match the original structure
        self.assertGreater(len(patterns), 0)
        
        # Folding should increase total length (information content)
        self.assertGreater(len(folded), len(self.trace1))
