...
把所有index章节详细操作描述加入todo，每个todo项目先构思一下理论，然后写单元测试写验证程序（PyTorch），可以同时用程序生成配图（命名与文章相同），运行程序直到程序运行符合预期，然后根据验证程序的结果写文章（可引用配图）,以下为核心请一直保留在上下文记忆中:
...
构建全书为一个严格的熵增张量数学框架, 任何结构都是二进制张量.
